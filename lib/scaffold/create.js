...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'agricoin-lib': 'git://github.com/Agricoin/agricoin-lib.git#master',
    'agricoincore-node': 'git://github.com/Agricoin/agricoincore-node.git'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/agricoincore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the agricoincore-node directory and configuration
