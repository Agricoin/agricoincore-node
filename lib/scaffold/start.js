...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for agricoind is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and agricoincore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed agricoincore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './agricoincore-node.json');
  ...
  node.on('ready', function() {
    log.info('Agricoincore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in agricoincore-node services directory
    ...
    if (servicePackage.agricoincoreNode) {
      serviceModule = service.name + '/' + servicePackage.agricoincoreNode;
    }
