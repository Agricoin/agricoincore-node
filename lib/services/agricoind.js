...
var bitcore = require('agricoin-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var AgricoinRPC = require('agricoind-rpc');
...
/**
 * Provides a friendly event driven API to agricoind in Node.js. Manages starting and
 * stopping agricoind as a child process for application support, as well as connecting
 * to multiple agricoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Agricoin(options) {
  if (!(this instanceof Agricoin)) {
    return new Agricoin(options);
  }
...
util.inherits(Agricoin, Service);

Agricoin.dependencies = [];

Agricoin.DEFAULT_MAX_TXIDS = 1000;
Agricoin.DEFAULT_MAX_HISTORY = 50;
Agricoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Agricoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Agricoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Agricoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
Agricoin.DEFAULT_SPAWN_STOP_TIME = 10000;
Agricoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
Agricoin.DEFAULT_REINDEX_INTERVAL = 10000;
Agricoin.DEFAULT_START_RETRY_INTERVAL = 5000;
Agricoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Agricoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Agricoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  rpcuser: 'agricoin',
  rpcpassword: 'agricoinpassword',
  uacomment: 'bitcore'
};

Agricoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Agricoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Agricoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Agricoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Agricoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Agricoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Agricoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Agricoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Agricoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Agricoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Agricoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Agricoin.prototype._initCaches = function() {
...
Agricoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Agricoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Agricoin.prototype.getPublishEvents = function() {
  return [
    {
...
Agricoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'agricoind/' + name, 'total:', this.subscriptions[name].length);
};

Agricoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'agricoind/' + name, 'total:', this.subscriptions[name].length);
};


Agricoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
Agricoin.prototype.unsubscribeBalance = function(emitter, addresses) {
...
Agricoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
Agricoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'agricoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Agricoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'agricoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Agricoin.prototype.unsubscribeAddressAll = function(emitter) {
...
Agricoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Agricoin.DEFAULT_CONFIG_SETTINGS;
...
Agricoin.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Agricoin.prototype._expandRelativeDatadir = function() {
...
Agricoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in agricoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in agricoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in agricoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './agricoin.conf');
  if (spawnOptions.configdir != null) {
    configPath = path.resolve(spawnOptions.configdir, './agricoin.conf');
  }

  log.info('Using agricoin config file:', configPath);
...
Agricoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of agricoincore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of agricoincore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of agricoincore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to agricoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from agricoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from agricoind. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that agricoind is undergoing a reindex. ' +
             ...
             'of agricoincore-node services will start.');
    node._reindex = true;
  }
};

Agricoin.prototype._resetCaches = function() {
...
Agricoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Agricoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Agricoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Agricoin Daemon Ready');
          callback();
        });
      });
...
Agricoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 5889
  };
...
Agricoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/agricoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/agricoin.conf';
    }
  }
  return networkPath;
};

Agricoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Agricoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('agricoind/hashblock', message.toString('hex'));
    }
  }

};

Agricoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Agricoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Agricoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Agricoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Agricoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Agricoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
Agricoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('agricoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Agricoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('agricoind/rawtransaction', message.toString('hex'));
    }
...
Agricoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Agricoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Agricoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Agricoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Agricoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Agricoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Agricoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Agricoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Agricoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/agricoind.pid';
...
Agricoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting agricoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Agricoin process unexpectedly exited with code:', code);
        log.warn('Restarting agricoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Agricoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn agricoind.'));
      }
...
Agricoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to agricoind.'));
    }
 ...
 */
Agricoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Agricoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Agricoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Agricoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
Agricoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, agricoincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Agricoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Agricoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Agricoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Agricoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in agricoincore is most recent to less recent
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Agricoin.prototype._getConfirmationsDetail = function(transaction) {
...
Agricoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Agricoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Agricoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A agricoin transaction id
 * @param {Function} callback
 */
Agricoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Agricoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Agricoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Agricoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Agricoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Agricoincore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Agricoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getInfo = function(callback) {
...
Agricoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('agricoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('agricoind process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Agricoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Agricoin.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Agricoin.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Agricoin.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Agricoin.prototype.getStakingInfo = function(callback) {
    var self = this,
